...
function starImages(rating) {
      //Creates an empty string variable as imageText
      let imageText = "";
      //For every time i is less than reviewers.length, repeat and add one star image for each point of the rating
      for (let i = 1; i <= rating; i++) {
            imageText += "<img src='images/star.png' alt=''>";
      }
      //Return the HTML string of star images
      return imageText;
}

//Add this function to be called after the document is fully loaded
window.addEventListener('DOMContentLoaded', (event) => {
      //Get the article element where the reviews will be inserted
      let article = document.getElementsByTagName("article")[0];
      //For every time i is less than reviewers.length, repeat
      for (let i = 0; i < reviewers.length; i++) {
            //Creates an empty string variable as reviewCode
            let reviewCode = "";
            //Add the appropriate table class based on the review type
            if (reviewType[i] === "P") {
            ...
            //Add the review content wrapped in HTML tags
            ...
            // Insert the review HTML into the article element
