...
document.getElementById("button1").onchange = function() {
   ...
   //FIX: Corrected "this.file[0]" to "this.files[0]"
...
function generateWordFreq(inputFile, outputDoc, outputCount) {
   ...
   //FIX: Changed "Reader" to "FileReader" 
   let fr = new FileReader();
   //FIX: Changed "fr.read" to "fr.readAsText"
   ...
   fr.onload=function() { 
      ...
      //FIX: Used "textContent" instead of "innerHTML" to get the source text
      let sourceText = outputDoc.textContent;
      
      // Remove any character that is not alphabetic or whitespace
      //FIX: Corrected the regex string to actual regex for removing characters
      ...
      //FIX: Added the global flag "g" to the regex pattern
      ...
      //FIX: Corrected loop condition from "i <= words.length" to "i < words.length"
      ...
      for (let i = 1; i <= 15; i++) {
         ...
         //FIX: Corrected percent calculation from "totalWords/freqs[i]" to "freqs[i]/totalWords"
